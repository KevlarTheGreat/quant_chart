...
class SecondaryRenderer extends BaseChartRenderer<MACDEntity> {
  ...
  void drawChart(MACDEntity lastPoint, MACDEntity curPoint, double lastX,
    ...
    switch (state) {
      ...
      case SecondaryState.RSI:
        drawLine(
            lastPoint.rsi, curPoint.rsi, canvas, lastX, curX, style.colors.rsi);
        break;

      case SecondaryState.WR:
        drawLine(
            lastPoint.r, curPoint.r, canvas, lastX, curX, style.colors.rsi);
        break;

      case SecondaryState.CCI:
        drawLine(
            lastPoint.cci, curPoint.cci, canvas, lastX, curX, style.colors.rsi);
        break;
  ...
  void drawMACD(MACDEntity curPoint, Canvas canvas, double curX,
    ...
    if (lastPoint.dif != 0)
      drawLine(
          lastPoint.dif, curPoint.dif, canvas, lastX, curX, style.colors.dif);
    if (lastPoint.dea != 0)
      drawLine(
          lastPoint.dea, curPoint.dea, canvas, lastX, curX, style.colors.dea);
  }
    ...
    switch (state) {
      case SecondaryState.MACD:
        children = [
          TextSpan(
              text: "MACD(12,26,9)    ",
              style: getTextStyle(style.colors.defaultText)),
          if (data.macd != 0)
            TextSpan(
                text: "MACD:${format(data.macd)}    ",
                style: getTextStyle(style.colors.macd)),
          if (data.dif != 0)
            TextSpan(
                text: "DIF:${format(data.dif)}    ",
                style: getTextStyle(style.colors.dif)),
          if (data.dea != 0)
            TextSpan(
                text: "DEA:${format(data.dea)}    ",
                style: getTextStyle(style.colors.dea))
        ];
        break;
      case SecondaryState.KDJ:
        children = [
          ...
          if (data.macd != 0)
            TextSpan(
                ...
                style: getTextStyle(style.colors.k)),
          if (data.dif != 0)
            TextSpan(
                text: "D:${format(data.d)}    ",
                style: getTextStyle(style.colors.d)),
          if (data.dea != 0)
            TextSpan(
                text: "J:${format(data.j)}    ",
                style: getTextStyle(style.colors.j))
        ];
        break;
      case SecondaryState.RSI:
        children = [
          TextSpan(
              ...
              style: getTextStyle(style.colors.rsi))
        ];
        break;
      case SecondaryState.WR:
        children = [
  ...
  void drawVerticalText(canvas, textStyle, int gridRows) {
    ...
    maxTp.paint(
        canvas,
        Offset(chartRect.width - maxTp.width - style.padding.right,
            chartRect.top - style.padding.top));
    minTp.paint(
        canvas,
        Offset(chartRect.width - minTp.width - style.padding.right,
  ...
  void drawGrid(Canvas canvas, int gridRows, int gridColumns) {
    ...
    for (int i = 0; i <= columnSpace; i++) {
      // Draw vertical lines in the secondary chart
      canvas.drawLine(
          Offset(columnSpace * i, chartRect.top - style.padding.top),
